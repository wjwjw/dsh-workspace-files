...
export type WorkspaceFilesFailure = WorkspaceFilePathUnavailable | WorkspaceFileNotDirectory | WorkspaceFileNotText | WorkspaceFileIsDirectory | WorkspaceFileDenied | WorkspaceFileIoError;
...
export type WorkspaceFileListResult = WorkspaceFilesSuccess<WorkspaceFileListing> | WorkspaceFilesRejected<WorkspaceFilePathUnavailable | WorkspaceFileNotDirectory | WorkspaceFileDenied | WorkspaceFileIoError>;
/** Result returned by the `read` operation. */
export type WorkspaceFileReadResult = WorkspaceFilesSuccess<WorkspaceFileRead> | WorkspaceFilesRejected<WorkspaceFilePathUnavailable | WorkspaceFileIsDirectory | WorkspaceFileNotText | WorkspaceFileDenied | WorkspaceFileIoError>;
/** Result returned by the `stat` operation. */
export type WorkspaceFileStatResult = WorkspaceFilesSuccess<WorkspaceFileStat> | WorkspaceFilesRejected<WorkspaceFilePathUnavailable | WorkspaceFileDenied | WorkspaceFileIoError>;
/** Result returned by the `write` operation (sandbox-gated like every fs mutation). */
export type WorkspaceFileWriteResult = WorkspaceFilesSuccess<WorkspaceFileWrite> | WorkspaceFilesRejected<WorkspaceFilePathUnavailable | WorkspaceFileIsDirectory | WorkspaceFileDenied | WorkspaceFileIoError>;
/** Result returned by the `reveal` operation (show-in-folder on the host desktop). */
export type WorkspaceFileRevealResult = WorkspaceFilesSuccess<{
    readonly path: string;
...
export type WorkspaceFileOpenResult = WorkspaceFilesSuccess<{
    readonly path: string;
}> | WorkspaceFilesRejected<WorkspaceFilePathUnavailable | WorkspaceFileDenied | WorkspaceFileIoError>;
