	...
	switch (platform) {
		case "win32": return {
			...
			args: [
				"-NoProfile",
				"-Command",
				`Explorer.exe '/select,${path.replace(/'/g, "''")}'`
			]
		};
		case "darwin": return {
			command: "open",
...
* @returns the shell-free command.
*/
function openCommand(path, platform = process.platform) {
	switch (platform) {
		case "win32": return {
			command: "powershell.exe",
			args: [
...
const READ_PREVIEW_BYTES = 256 * 1024;
...
function sortEntries(entries) {
	return [...entries].sort((left, right) => kindRank(left.kind) - kindRank(right.kind) || left.name.localeCompare(right.name, void 0, { sensitivity: "base" }));
}
...
function mapFsError(error, path) {
	if (error instanceof FsError) switch (error.code) {
		...
		case "FS_NOT_DIRECTORY": return {
			code: "not-a-directory",
			path
		};
		case "FS_NOT_TEXT":
		case "FS_NOT_REGULAR_FILE": return {
			code: "not-a-text-file",
			path
		};
...
function narrowFailure(error, path, codes) {
	if (codes.includes(error.code)) return error;
	return {
		code: "io-error",
		path
	};
...
let WorkspaceFilesGateway = (() => {
	...
	return class WorkspaceFilesGateway extends _classSuper {
		static {
			...
			__esDecorate(this, null, _reveal_decorators, {
				kind: "method",
				name: "reveal",
				static: false,
				private: false,
				access: {
					has: (obj) => "reveal" in obj,
					get: (obj) => obj.reveal
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		*/
		async list(path, signal) {
			...
			try {
				...
				if (info === void 0) return rejected({
					code: "path-unavailable",
					path: requested
				});
				if (info.type !== "directory") return rejected({
					code: "not-a-directory",
					path: requested
				});
				const children = await this.ctx.fs.listDir(target, signal);
				const [root, entries] = await Promise.all([hostRoot(this.ctx, signal), Promise.all(children.map(async (child) => {
				...
				return rejected(narrowFailure(mapFsError(error, requested), requested, [
					"path-unavailable",
					"not-a-directory",
					"permission-denied",
					"io-error"
		...
		*/
		async read(path, signal) {
			try {
				...
				if (info.type !== "file") return rejected({
					code: "not-a-text-file",
					path
				});
				if (info.size !== void 0 && info.size > 262144) return success({
					content: "",
					truncated: true,
				...
				return rejected(narrowFailure(mapFsError(error, path), path, [
					"path-unavailable",
					"is-a-directory",
					"not-a-text-file",
					"permission-denied",
		...
		*/
		async write(path, content, signal) {
			try {
				const target = await this.ctx.fs.resolve(path, resolveOptions(signal));
				const info = await this.ctx.fs.stat(target, signal);
				if (info !== void 0 && info.type === "directory") return rejected({
					code: "is-a-directory",
					path
				});
				return success({
					operation: (await this.ctx.fs.writeText(target, content, void 0, signal)).operation,
					path: this.ctx.fs.processPath(target),
...
//#endregion
export { READ_PREVIEW_BYTES, WorkspaceFilesGateway, WorkspaceFilesGateway as default, openCommand, revealCommand };
